...
public class UserDao {
   ...
   public user login(String id, String password) throws Exception {
      ...
      user users =  jdbcTemplate.queryForObject(sql, new Object [] {id, password},new UserMapper());
      return users;
   }
   
   //sign up
   public void create(user user) {
      String sql = "insert into user(id, role, nick, fixnic, password, name, mail, mail01, coin) values (?, ?, ?, ?, ?, ?, ?, ?, ?)";
      jdbcTemplate.update(sql, user.getId(),user.getRole(),user.getNick(),user.getFixnic(),user.getPassword(),user.getName(),user.getMail(), user.getMail01(),user.getCoin());
   }
